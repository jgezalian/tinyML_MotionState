...
def plot_all_attributes(attributes, accel, decel, accel_path, decel_path):
    for attribute in attributes:
        ...
        plt.grid(True)
        plt.savefig(f"{decel_path}/{attribute}.png")
        plt.close()
        ...
        fig.canvas.manager.set_window_title(attribute)
        plt.grid(True)
        plt.savefig(f"{accel_path}/{attribute}.png")
        plt.close()
...
def clean(df, flag):
    ...
    if flag == "train":
        accel_path = "../drive_data/clean/train/smooth_accel"
        decel_path = "../drive_data/clean/train/smooth_decel"

    elif flag == "test":
        accel_path = "../drive_data/clean/test/smooth_accel"
        decel_path = "../drive_data/clean/test/smooth_decel"
    ...
    decel.to_csv(f"{decel_path}/smooth_decel.csv")
    ...
    accel.to_csv(f"{accel_path}/smooth_accel.csv")

    plot_all_attributes(attributes, accel, decel, accel_path, decel_path)
