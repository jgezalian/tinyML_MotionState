...
csv = "../drive_data/first_and_second_combined_raw/constant_speed/constant_speed.csv"
df = pd.read_csv(csv)

df["time_sec"] = (df["timestamp"] - df.iloc[0]["timestamp"]) / 1000

df.to_csv("../drive_data/clean/constant_speed/constant_speed.csv", index=False)
...
def plot_all_attributes(attributes):
    for attribute in attributes:
        ...
        plt.title(attribute)
        plt.savefig(f"../drive_data/clean/constant_speed/{attribute}.png")
