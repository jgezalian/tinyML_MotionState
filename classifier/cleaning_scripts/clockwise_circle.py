...
# clockwise_loop
csv = "../drive_data/second_set_raw/clockwise_circle.csv"
df = pd.read_csv(csv)
time_col = "timestamp"
df["time_sec"] = (df[time_col] - df[time_col].iloc[0]) / 1000
#df = df[(df["time_sec"] >= 2) & (df["time_sec"] <= 115)]
df.to_csv("../drive_data/second_set_clean/clockwise_circle/clockwise_circle.csv")
...
def plot_all_attributes(attributes):
    for attribute in attributes:
        ...
        plt.title(attribute)
        plt.savefig(f"../drive_data/second_set_clean/clockwise_circle/{attribute}.png")
